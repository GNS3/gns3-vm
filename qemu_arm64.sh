...
if [ ! -f "ubuntu-20.04.3-live-server-arm64.iso" ]
then
  wget https://cdimage.ubuntu.com/releases/20.04/release/ubuntu-20.04.3-live-server-arm64.iso
fi

mkdir -p ubuntu-arm64-iso
sudo mount -r ubuntu-20.04.3-live-server-arm64.iso ubuntu-arm64-iso

sudo apt update && sudo apt install -y qemu-utils qemu-system-arm qemu-efi-aarch64
...
-kernel iso/casper/vmlinuz -initrd iso/casper/initrd \
-append "autoinstall ds=nocloud-net;s=http://10.0.2.2:4242/ console=ttyAMA0" \
-cdrom ubuntu-20.04.3-live-server-arm64.iso -no-reboot -boot strict=off
