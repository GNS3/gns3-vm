...
if [ ! -f "ubuntu-20.04.6-live-server-arm64.iso" ]
then
  wget https://cdimage.ubuntu.com/releases/20.04/release/ubuntu-20.04.6-live-server-arm64.iso
fi

mkdir -p ubuntu-arm64-iso
mount -r ubuntu-20.04.6-live-server-arm64.iso ubuntu-arm64-iso
...
-cdrom ubuntu-20.04.6-live-server-arm64.iso -no-reboot -boot strict=off
