...
export GNS3_SRC="/tmp/GNS3VM.KVM.${GNS3VM_VERSION}.ova"

if [[ "$GNS3_VM_FILE" == "" ]]
then
    export GNS3VM_VERSION="0.10.17" # `python last_vm_version.py`
    ...
    if [[ ! -f "/tmp/GNS3VM.KVM.${GNS3VM_VERSION}.zip" ]]
    then
        echo "Downloading $GNS3VM_URL"
        curl -Lk "$GNS3VM_URL" > "/tmp/GNS3VM.KVM.${GNS3VM_VERSION}.zip"
    fi
else
    echo "GNS3 VM file: $GNS3_VM_FILE"
    export GNS3VM_VERSION=`cat version`
    cp "$GNS3_VM_FILE" "/tmp/GNS3VM.KVM.${GNS3VM_VERSION}.zip"
fi
unzip -p "/tmp/GNS3VM.KVM.${GNS3VM_VERSION}.zip" "GNS3 VM.ova" > ${GNS3_SRC}
...
zip -9 "GNS3 VM KVM ${GNS3_VERSION}.zip" *.qcow2
